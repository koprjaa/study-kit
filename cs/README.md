...
2. **Rozbal oficiální šablonu FIS** (BP-DP.ZIP z intranetu, obsah adresáře
   `SablonaBP-DP/`) do `study/` — kit pak edituje kapitolové soubory šablony,
   `literatura.bib` a `prohlaseniAI.tex`.
3. **Vlož artefakty do `raw_primary/`** a literaturu do `raw_secondary/` —
   obojí může zpočátku být skoro prázdné, artefakty přibývají s experimenty.
4. **Vyplň `idea.md`** — vhled, oficiální zadání, program, rozsah.
5. **Spusť Claude Code ve složce:** `claude`. Načte `CLAUDE.md` automaticky.
6. **Napiš `/teze`** (nebo jen „začni"). Claude tě povede fázi po fázi,
