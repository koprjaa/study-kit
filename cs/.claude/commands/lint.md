...
- uvádí každá tabulka/graf zdrojový artefakt z `raw_primary/`? má `\caption`,
  `\label` a odkaz `\ref` z textu?
...
- má každý `\cite`/`\parencite`/`\textcite` klíč položku v `study/literatura.bib`
  a obráceně (žádné necitované položky)? existuje každý zdroj v `raw_secondary/`?
