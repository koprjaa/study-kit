...
- **Kapitoly jsou soubory šablony** (`uvod.tex`, `kapNN-nazev.tex`, `zaver.tex`) —
  jen obsah, žádné preambule. Novou kapitolu přidej do `\include` seznamu
  v `prace.tex`. Šablona používá biblatex `style=apa` + biber a `\cite` mapuje
  na `\parencite`.
- **Bibliografie:** `study/literatura.bib` (soubor šablony), vede ji `/reserse`.
  Klíč = `prijmeniROK` (např. `novak2024`). Smí obsahovat VÝHRADNĚ zdroje
  z `raw_secondary/` (+ citované AI výstupy, viz AI transparentnost).
...
## AI transparentnost (pravidla VŠE + citace dle CIKS)

Tenhle kit JE použití GenAI — a VŠE vyžaduje jeho transparentní deklaraci.
Neupravený a necitovaný výstup AI v práci se posuzuje jako **podvod**; plnou
odpovědnost za obsah nese student.

- **`process/ai-log.md` vedeš průběžně** — po každé fázi připiš záznam: nástroj
  (Claude Code + model), která část práce, způsob použití (rešerše/draft/revize/
  kód/…) a **co je vlastní přínos člověka** (rozhodnutí, teze, výběry, korekce).
  Bez téhle evidence se příloha na konci nedá poctivě sestavit.
- **`/prohlaseni`** z `ai-log.md` vygeneruje: (a) doporučení, které kategorie
  v `prohlaseniAI.tex` nechat, (b) draft přílohy „Použití nástrojů AI" — které
  použití, kde, jakým nástrojem, se zvýrazněným vlastním přínosem (vyžaduje
  šablona i pravidla VŠE).
- **Citace AI výstupu (CIKS, APA 7):** jen pokud je výstup GenAI v textu citován
  či parafrázován → v textu `(Tvůrce AI, rok)`, v `literatura.bib` položka
  nástroje (Tvůrce. (rok, den. měsíc). *Název* (verze) [popis]. URL), u chatu
  název chatu + sdílený odkaz (archivovat). Podklady nevyužité doslovně →
  deklarace v příloze/metodice, ne citace. Čistě formální úpravy (pravopis,
  bibliografické nástroje) → deklarovat netřeba.
- **Data do AI:** nevkládat osobní/citlivé/autorsky chráněné materiály bez
  právních záruk nástroje (pravidlo VŠE č. 1). Že nástroj záruky má, si ověří
  a rozhodne člověk — týká se i literatury v `raw_secondary/`.
...
- **Jedna fáze = jeden krok.** Kandidáti → předlož → otázka gate → počkej na
  rozhodnutí → zapiš artefakt → řádek do `log.md` + záznam do
  `process/ai-log.md` → navrhni další krok.
...
### Bonus · `/zaver`, `/abstrakt`, `/oponent`, `/prohlaseni`
...
**B.4 AI prohlášení · `/prohlaseni`.** Z `process/ai-log.md` sestav podklady pro
`prohlaseniAI.tex` (které kategorie nechat) a draft přílohy „Použití nástrojů AI"
(nástroj → část práce → způsob → zvýrazněný vlastní přínos; viz AI transparentnost).
Neúplný `ai-log.md` → nahlas mezery, nedomýšlej je.
...
- **`/lint`** — projdi `process/` a `study/`, nahlas checklist (neopravuj
  ...
  - je `process/ai-log.md` aktuální (pokrývá všechny proběhlé fáze)? je vyplněné
    `prohlaseniAI.tex` + příloha o použití AI?
